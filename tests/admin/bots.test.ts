...
describe('Admin - Bot Management Tests', () => {
  ...
  it('Import bot from existing archive', async () => {
    page.waitForSelector('#btn-create-bot',{visible: true})
...
//else console.log("not null") 
    ...
    expect(await page.$(`a[href="studio/${importBotId}"]`)).toBeNull()//verifies the link does not exist
  })
  ...
  it('Create a brand new  bot', async () => {
    ...
    await expect(page.$(`a[href*="studio2/${tempBotId}"]`)[0]).not.toBeNull()  //verifies the link to the new bot
  ...
  it('Verify the new bot has been successfully exported', async () => {
    ...
    //the following code verifies through the API
   ...
   // expect(responseSize).toBeGreaterThan(100)
    ...
    async function MyFlag(path,botname){
  ...
  it('Create revision, then roll it back and then confirms in the logs the roll back', async () => {
    ...
    //await page.waitForXPath('//span[class="bp3-toast-message"][contains(., "Revisions created")]',{visible: true}) 
